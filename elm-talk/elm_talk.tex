...
\end{itemize}
\[
\frac{\Gamma\vdash e:\Sigma\alpha\quad\quad\Gamma\vdash c:\alpha}{\Gamma\vdash e^{\left[c\right]}:\Sigma\alpha}\:\textsc{CurrentEval}
\]

\begin{itemize}
\item Every predefined signal $i:\Sigma\alpha$ has an initial value: $i^{[a]}$
\item \textbf{Initial} current values for combinators are evaluated:
\end{itemize}
\begin{align*}
\frac{\Gamma\vdash\mbox{constant}\ c:\Sigma\alpha}{\Gamma\vdash\left(\mbox{constant}\ c\right)^{\left[c\right]}:\Sigma\alpha} & \:\textsc{ConstInit}\\
\frac{\Gamma\vdash\mbox{map2}\ m\ p^{\left[a\right]}\ q^{\left[b\right]}:\Sigma\gamma}{\Gamma\vdash\left(\mbox{map2}\ m\ p\ q\right)^{\left[m\ a\ b\right]}:\Sigma\gamma} & \:\textsc{Map2Init}\\
\frac{\Gamma\vdash\mbox{foldp}\, u\, e\, q:\Sigma\beta}{\Gamma\vdash\left(\mbox{foldp}\, u\, e\, q\right){}^{\left[e\right]}:\Sigma\beta} & \:\textsc{FoldPInit}
\end{align*}
...
\begin{itemize}
\item Update steps happen only to \textbf{input signals} $s$ and \textbf{one
at a time}
...
\begin{itemize}
\item An update step on $s$ will leave all other signals unchanged:
\end{itemize}
\[
\mathbf{U}_{s\leftarrow b}\left\{ s^{\left[a\right]}\right\} \Rightarrow s^{\left[b\right]}\quad\quad\mathbf{U}_{s\leftarrow b}\left\{ s'^{\left[c\right]}\right\} \Rightarrow s'^{\left[c\right]}
\]
...
\begin{itemize}
\item The whole program \textbf{remains} a static signal expression tree
\item Only the current values are updated in all subexpressions
\end{itemize}
\begin{align*}
 & \mathbf{U}_{s\leftarrow a}\left\{ \mbox{constant}\ c\right\} \Rightarrow\left(\mbox{constant}\ c\right)^{\left[c\right]} & \textsc{ConstUpd}\\
 & \mathbf{U}_{s\leftarrow a}\left\{ \mbox{map2}\ m\ p\ q\right\} \\
 & \quad\Rightarrow\left(\mbox{map2}\ m\ \mathbf{U}_{s\leftarrow a}\left\{ p\right\} ^{\left[b\right]}\ \mathbf{U}_{s\leftarrow a}\left\{ q\right\} ^{\left[c\right]}\right)^{\left[m\ b\ c\right]} & \textsc{Map2Upd}\\
...
after\_unless~(b,r)~w~=~(w~or~b)~and~not~r
...
\item Use \texttt{async} : $\Sigma\alpha\rightarrow\Sigma\alpha$
...
\begin{itemize}
\item UI shows results of long computations
\end{itemize}
