...
\end{itemize}
\item Asynchronous (out-of-order) callback structures are difficult to maintain
...
\begin{itemize}
\item Explicit multithreaded code is hard
\end{itemize}
\end{itemize}
...
\item Standard library with data structures, HTML, HTTP, etc.
...
main~=~constant~(~plainText~\textquotedbl{}Hello,~World!\textquotedbl{}~)
\end{lyxcode}
...
\begin{itemize}
\item A manager that merges the required input signals:\end{itemize}
\begin{lyxcode}
merge\_input:~$\Sigma$Command\end{lyxcode}
\begin{itemize}
\item Predefined input signals: Mouse, Keyboard, timer, HTML, etc.
...
main~=~map~draw~\$~foldp~update~init\_state~merge\_input~
\end{lyxcode}
...
\item No higher-order signals: no $\Sigma(\Sigma\alpha)$ allowed by type
system
\item The signal processing logic is specified statically
\item No way of defining new signals
