...
\begin{itemize}
\item Fix initial value semantics for \texttt{async'} : $\alpha\rightarrow\Sigma\alpha\rightarrow\Sigma\alpha$ 
...
s~=~async'~s0~(map~f~s)
\end{lyxcode}
\item Add monadic signal combinator, \texttt{bind :} $(\alpha\rightarrow\Sigma\beta)\rightarrow\Sigma\alpha\rightarrow\Sigma\beta$
