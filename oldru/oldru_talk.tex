...
\begin{itemize}
\item motivation: ``simplify the spelling''
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Omit the silent ``e'' in endings, replace vowels to simplify spelling

\begin{itemize}
\item breathe $\rightarrow$ breath, some $\rightarrow$ sum, made $\rightarrow$
maid, gate $\rightarrow$ gait
\end{itemize}
\item Replace the letters c$\rightarrow$k/s, x$\rightarrow$ks, y$\rightarrow$i

\begin{itemize}
\item played $\rightarrow$ plaid, hence $\rightarrow$ hens, xerox $\rightarrow$
kseroks
\end{itemize}
\item Physically remove the letters C, X, Y from all computer keyboards
...
orthography
\item Such texts are perceived to be unquestionably obsolete
\item No typewriters \textbf{or computers} support the old Russian orthography 
\end{itemize}
\end{itemize}
\end{frame}
...
\item Misses some words, needs manual check
\end{itemize}
