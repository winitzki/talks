...
Molecule \emph{emitters} are values of type \texttt{\textcolor{blue}{\footnotesize{}M{[}A{]}}}:
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}val~count~=~m{[}Int{]};~val~inc~=~m{[}Unit{]}}{\footnotesize\par}
\end{lyxcode}
Molecule \emph{instances} are emitted by calling the emitter's \texttt{\textcolor{blue}{\footnotesize{}apply}}
method:
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}count(0);~inc();~inc();~inc();}\textcolor{gray}{\scriptsize{}~//~Side-effect:~emit~molecules.}{\scriptsize\par}
\end{lyxcode}
...
clause
\item All molecules (e.g., \texttt{\textcolor{blue}{\footnotesize{}count}},
\texttt{\textcolor{blue}{\footnotesize{}inc}}) must be already defined
\end{itemize}
A group of reactions are ``activated'' within a ``reaction site'':
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}site(r1,~r2,~r3)}\textcolor{gray}{\scriptsize{}~//~Side-effect:~activate~reactions~r1,~r2,~r3.}{\scriptsize\par}
\end{lyxcode}
\begin{itemize}
\item A molecule may be emitted only after activating a reaction site where
some reactions consume that molecule as input
\item A site \emph{must} include all reactions that consume a given molecule
\end{itemize}
A Chemical Machine program declares some molecules, reactions, and
reaction sites, and then can emit some initial molecules
\begin{itemize}
...
\textcolor{blue}{\footnotesize{}~val~enable~=~m{[}Unit{]}~}\textcolor{gray}{\scriptsize{}//~Emitter~is~confined~to~the~local~scope.}{\scriptsize\par}
...
\end{lyxcode}
\begin{itemize}
\item No threads (green or not), no semaphores, no locks, no mutable state
...
Assume a molecule \texttt{\textcolor{blue}{\footnotesize{}p}} is already
bound in our reaction(s)

Create a ``throttled version'' \texttt{\textcolor{blue}{\footnotesize{}tp}}
that has the same effect as \texttt{\textcolor{blue}{\footnotesize{}p}}
except that \texttt{\textcolor{blue}{\footnotesize{}tp}} is always
emitted at a limited rate

To test, emit \texttt{\textcolor{blue}{\footnotesize{}tp}} several
times quickly:
\begin{lyxcode}
...
\textcolor{blue}{\footnotesize{}val~pt~=~throttle(p,~1000L)}\textcolor{gray}{\scriptsize{}~//~1~second~minimum~delay.}{\scriptsize\par}

\textcolor{blue}{\footnotesize{}pt(10);~pt(20);~pt(30);}{\footnotesize\par}
...
\begin{frame}{Example: async cancellation}

Prevent emission of molecule \texttt{\textcolor{blue}{\footnotesize{}x}}
...
\begin{frame}{Example: async cancellation test}

Assume a molecule \texttt{\textcolor{blue}{\footnotesize{}x}} is already
bound in our reaction(s)

Create a ``cancellable version'' \texttt{\textcolor{blue}{\footnotesize{}cx}}
that has the same effect as \texttt{\textcolor{blue}{\footnotesize{}x}}
but can be (eventually) canceled by emitting a molecule \texttt{\textcolor{blue}{\footnotesize{}cancel}}:
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}val~x~=~m{[}Int{]}}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}site(~go~\{~case~x(a)~\ensuremath{\Rightarrow}~println(s\textquotedbl Got~\$a\textquotedbl )~\}~)}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}val~(cx,~cancel)~=~cancellation(x)}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}scala>~cx(1);~cx(2);~cx(3);~cancel();~cx(4);~cx(5);}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}Got~1}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}Got~2}{\footnotesize\par}
\end{lyxcode}
\emph{Asynchronous} cancellation cannot guarantee order of events!
\end{frame}

\begin{frame}{Example: async racing}

For two molecules \texttt{\textcolor{blue}{\footnotesize{}p}} and
\texttt{\textcolor{blue}{\footnotesize{}q}}, determine which one was
emitted first and report its value on a given molecule (\texttt{\textcolor{blue}{\footnotesize{}reply}}):
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}def~racing{[}A,~B{]}(reply:~M{[}Either{[}A,~B{]}{]}):~(M{[}A{]},~M{[}B{]})~=~\{}{\footnotesize\par}
...
\textcolor{blue}{\footnotesize{}~~~~go~\{~case~p(a)~+~enable(\_)~\ensuremath{\Rightarrow}~reply(Left(a))~~\},}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~~~go~\{~case~q(b)~+~enable(\_)~\ensuremath{\Rightarrow}~reply(Right(b))~\},}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~)}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~enable()~}\textcolor{gray}{\scriptsize{}//~Initially,~we~only~have~one~copy~of~`enable`.}{\scriptsize\par}
...
and \texttt{\textcolor{blue}{\footnotesize{}q(...)}}, will get \texttt{\textcolor{blue}{\footnotesize{}reply(...)}}
emitted
\item External code cannot emit \texttt{\textcolor{blue}{\footnotesize{}enable()}}
because of local scope
\item Order of async molecules is not guaranteed
\end{itemize}
...
\begin{frame}{Example: async racing test}

Assume a molecule \texttt{\textcolor{blue}{\footnotesize{}result}}
is already bound in our reaction(s)
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}val~result~=~m{[}Either{[}Int,~String{]}{]}}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}site(}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~\{~go~\{~case~result(Left(x))~~\ensuremath{\Rightarrow}~...~\},}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~\{~go~\{~case~result(Right(y))~\ensuremath{\Rightarrow}~...~\},}{\footnotesize\par}

\textcolor{blue}{\footnotesize{})}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}val~(p,~q)~=~racing{[}Int,~String{]}(result)}{\footnotesize\par}

\textcolor{gray}{\scriptsize{}//~Two~parallel~computations~can~now~emit~`p`~and~`q`~asynchronously.}{\scriptsize\par}

\textcolor{gray}{\scriptsize{}//~First~computation:}{\scriptsize\par}

\textcolor{blue}{\footnotesize{}...~p(123)}\textcolor{gray}{\scriptsize{}~~~~//~Non-blocking.}{\scriptsize\par}

\textcolor{gray}{\scriptsize{}//~Second~computation:}{\scriptsize\par}

\textcolor{blue}{\footnotesize{}...~q(\textquotedbl abc\textquotedbl )}\textcolor{gray}{\scriptsize{}~//~Non-blocking.~}{\scriptsize\par}

\textcolor{gray}{\scriptsize{}//~Eventually,~a~`result(...)`~molecule~will~be~emitted.}{\scriptsize\par}
\end{lyxcode}
\end{frame}
...
\begin{lyxcode}
\textcolor{blue}{\scriptsize{}val~fetch~=~b{[}Int,~String{]}}\textcolor{gray}{\scriptsize{}~//~Blocking~molecule's~emitter.}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}val~data~=~m{[}Int{]}}\textcolor{gray}{\scriptsize{}~//~A~non-blocking~molecule~emitter.}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}site(}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}~~go~\{~case~data(x)~+~fetch(y,~reply)~}\textcolor{blue}{\footnotesize{}\ensuremath{\Rightarrow}}\textcolor{blue}{\scriptsize{}~reply(s\textquotedbl got~\$x~and~\$y\textquotedbl )~\}}{\scriptsize\par}

\textcolor{blue}{\scriptsize{})}\textcolor{gray}{\footnotesize{}~}\textcolor{gray}{\scriptsize{}//~`reply`~is~a~ReplyEmitter{[}String{]}~defined~within~reaction~scope.}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}data(123)}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}val~x~=~fetch(456)}\textcolor{gray}{\footnotesize{}~}\textcolor{gray}{\scriptsize{}//~Blocking~call,~will~set~x~=~\textquotedbl got~123~and~456\textquotedbl .}{\scriptsize\par}
\end{lyxcode}
~
...
\begin{frame}{Example: synchronous rendez-vous}

Create two blocking molecules \texttt{\textcolor{blue}{\footnotesize{}p(x:~X)}}
and \texttt{\textcolor{blue}{\footnotesize{}q(y:~Y)}} to implement
synchronous rendez-vous that exchanges \texttt{\textcolor{blue}{\footnotesize{}x}}
and \texttt{\textcolor{blue}{\footnotesize{}y}}:
\begin{lyxcode}
\textcolor{blue}{\footnotesize{}def~syncRendezvous{[}X,~Y{]}:~(B{[}X,~Y{]},~B{[}Y,~X{]})~=~\{}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~val~p~=~b{[}X,~Y{]};~val~q~=~b{[}Y,~X{]}}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~site(~go~\{~case~p(x,~rx)~+~q(y,~ry)~\ensuremath{\Rightarrow}~rx(y);~ry(x)~\}~)}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}~~(p,~q)}{\footnotesize\par}

\textcolor{blue}{\footnotesize{}\}}{\footnotesize\par}

\textcolor{gray}{\scriptsize{}//~To~test:~Create~the~molecules~for~a~rendez-vous.}{\scriptsize\par}

\textcolor{blue}{\footnotesize{}val~(p,~q)~=~syncRendezvous{[}Int,~String{]}}{\footnotesize\par}

\textcolor{gray}{\scriptsize{}//~Two~parallel~computations~can~exchange~data.~First~computation:}{\scriptsize\par}

\textcolor{blue}{\footnotesize{}...~val~myResponse:~String~=~p(123)}\textcolor{gray}{\scriptsize{}~//~Blocking~until~response.}{\scriptsize\par}

\textcolor{gray}{\scriptsize{}//~Second~computation:}{\scriptsize\par}

\textcolor{blue}{\footnotesize{}...~val~myResponse:~Int~=~q(\textquotedbl abc\textquotedbl )~}\textcolor{gray}{\scriptsize{}~//~Blocking~until~response.}{\scriptsize\par}
\end{lyxcode}
\end{frame}
...
to determine which of \texttt{\textcolor{blue}{\footnotesize{}p}}
and \texttt{\textcolor{blue}{\footnotesize{}q}} returns first and
return the corresponding value of type \texttt{\textcolor{blue}{\footnotesize{}Either{[}X,
Y{]}}}:
\begin{lyxcode}
...
\end{lyxcode}
\begin{itemize}
\item External code calls \texttt{\textcolor{blue}{\footnotesize{}r(...)}},
which will call both \texttt{\textcolor{blue}{\footnotesize{}p(...)}}
and \texttt{\textcolor{blue}{\footnotesize{}q(...)}}{\footnotesize\par}
\end{itemize}
Libraries such as Monix or ZIO provide a racing facility
\end{frame}

\begin{frame}{Example: parallel map/reduce}
...
\textcolor{gray}{\scriptsize{}~~//~``map''~to~perform~a~long~computation:}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}~~go~\{~case~c(x)~$\Rightarrow$~d((1,~long\_computation(x)))~\},~}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}~}\textcolor{gray}{\scriptsize{}~//~``reduce''~to~aggregate~the~results:}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}~~go~\{~case~d((n1,~b1))~+~d((n2,~b2))~$\Rightarrow$}{\scriptsize\par}

\textcolor{blue}{\scriptsize{}~~~val~(newN,~newB)~=~(n1~+~n2,~b1~|+|~b2)}\textcolor{gray}{\scriptsize{}~//~Aggregation.}{\scriptsize\par}
...
\textcolor{blue}{\scriptsize{}~~~~~~~~}\textcolor{brown}{\scriptsize{}site}\textcolor{blue}{\scriptsize{}(~~}\textsf{\textcolor{gray}{\footnotesize{}~}}\textcolor{gray}{\scriptsize{}//~Define~a~lower-level~reaction~site.}{\scriptsize\par}
...
\textcolor{brown}{\scriptsize{}~~~~~~~~~~~~~sortedResult}\textcolor{blue}{\scriptsize{}(arrayMerge(x,~y))~}\textcolor{gray}{\scriptsize{}//~Helper~function.}{\scriptsize\par}
...
\textcolor{blue}{\scriptsize{}~~~~~~~~}\textcolor{gray}{\scriptsize{}//~Emit~lower-level~`mergesort`~molecules:}{\scriptsize\par}
...
(barriers, critical sections, readers/writers, Conway's ``Game of
Life'', elevator control, etc.)
\end{frame}
...
\item Reaction sites are immutable; input molecule sets are defined statically
...
A \href{https://winitzki.gitbooks.io/concurrency-in-reactions-declarative-multicore-in/content/}{tutorial book}
goes through many examples
...
\end{itemize}
\item Logging, metrics monitoring, debugging, unit-testing facilities
\end{itemize}
Possible extensions:
\begin{itemize}
\item Persistence?
\item Automatic parallelism adjustment?
\end{itemize}
\end{frame}

\begin{frame}{Comparison to Petri nets}
