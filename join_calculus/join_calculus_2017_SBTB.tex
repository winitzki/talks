...
\title[Declarative Concurrency with Chymyst]{Declarative Concurrent Programming with Chymyst}
...
\item available as an \href{https://github.com/Chymyst/chymyst-core}{open-source library \&{} DSL}
\end{itemize}
\end{itemize}
\end{frame}
...
+ c(y) $\Rightarrow$ }}\texttt{\textcolor{olive}{\scriptsize{}reply}}\texttt{\textcolor{blue}{\scriptsize{}(s''\$\{x
+ y\}'') \}}}~\\
...
\item actors will consume one message at a time
...
\item many molecules can be consumed at once, atomically
...
Reaction scheduler in \emph{Chymyst}:
\begin{itemize}
...
\begin{itemize}
\item This depends on the specific chemical program
\end{itemize}
\item If so, molecule instances can be held in an ordered queue (``pipelined'')
\item \texttt{Chymyst} \emph{automatically} makes some molecules pipelined
\end{itemize}
...
\begin{tabular}{|c|c|c|}
\hline 
\textbf{``Chemistry''} & \textbf{JC terminology} & \texttt{\textbf{Chymyst}}\textbf{ code}\tabularnewline
\hline 
\hline 
