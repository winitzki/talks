...
build upon the existing Actor model knowledge. However, reasoning
about JC programs is most direct and convenient when using the visual
metaphors and the terminology of the Abstract Chemical Machine (``emitter''
/ ``molecule'' / ``reaction'').

The next subsection is a brief introduction to programming in join
calculus using \texttt{Chymyst}. This introduction is suitable for
readers not already familiar with either join calculus or the Actor
model.
...
\end{align*}
and result in creating new \textbf{molecule emitters} \texttt{c} and
\texttt{t}. Emitters can be seen as functions that are called in order
...
clause, wrapped into an auxiliary method called \texttt{go()}:
\[
...
one output molecule, \texttt{c}, with a computed new value \texttt{xs}.
The effect of this reaction is to compute the tail of a list. Due
to the pattern-matching requirement, this reaction will start only
when the molecule \texttt{c} carries a non-empty list value.

In \texttt{Chymyst}, reaction definitions can use all features of
Scala partial functions, including arbitrary guard conditions and
pattern matching on molecule values, and include arbitrary Scala code
...
\]
Creating the reaction value \texttt{r1} does not actually schedule
a computation; it merely defines the available computation declaratively.
In order to make the chemical machine run reactions, the programmer
needs to create a reaction site using the \texttt{site()} call, such
as \texttt{site(r1, r2)}, which activates the reactions listed in
its arguments. A reaction site typically includes several reactions
that may be declared inline for brevity:
\begin{align*}
...
such as \texttt{t} or \texttt{c}, or molecules bound to other reaction
...
a continuation results in ``scope ripping,'' which disrupts the
normal code flow. 

The JoCaml implementation of JC uses synchronous channels to mitigate
this problem. \texttt{Chymyst} also implements synchronous channels
as a language feature. \texttt{Chymyst's} chosen syntax for synchronous
channels resembles continuation-passing style and looks like this:
\[
...
Here is a more extended example of a \texttt{Chymyst} process with
...
continuation argument \texttt{cont}, which is nevertheless available
in the reaction body. The semantics of the blocking call \texttt{f(123)}
is essentially to construct the current continuation as \texttt{cont}
and to send it to the process as a message on the channel \texttt{f},
together with the ordinary payload \texttt{123}. 
...
channels, enables a declarative implementation of \textbf{synchronous
...
users to specify the copy of \texttt{f()} to which the reply is sent.
...
philosophers by creating $n$ philosopher and fork channels at run
time, defining an array of $n$ processes for these channels, and
aggregating the $n$ processes into a join definition. 
...
that the process waits on the channels \texttt{t} and \texttt{c},
that the channel \texttt{t} may have arbitrary message values while
\texttt{c} requires a pattern match, and that the process will finally
send a message on channel \texttt{c} but not on \texttt{t}.
...
Unavoidable nondeterminism within a join definition occurs when one
process waits on a subset of messages that another process is also
waiting on, for instance
\begin{align*}
...
\]
In many cases, this transformation allows the runtime engine to select
message values faster, without enumerating all combinations of message
values in a search for suitable inputs for a process.

\subsubsection{Time-outs}

When a synchronous message is sent, the emitting process is blocked
until a reply is received. In \texttt{Chymyst}, it is possible to
specify a time-out for this blocking call: 
\[
\text{\texttt{f.timeout()(2 seconds)}}
\]
A reaction that sends the reply can also check whether the waiting
process received the reply value or timed out. This is implemented
as a \texttt{Boolean} return value from the reply emitter:
\[
\text{\texttt{go \{ case f(\_,r) \ensuremath{\Rightarrow} if (r("done")) ...~\}}}
\]
The timeout-checking functionality may be sometimes required to avoid
race conditions when using timeouts on synchronous channels.

\subsection{What it means to be ``industry ready''{*}{*}{*}}
...
of the computation \texttt{c1} concurrently. The runtime engine may
automatically adjust the degree of parallelism depending on CPU load.
...
what needs to be done with any message sent via \texttt{c1}. We could
say that the value \texttt{c1} plays the role of a \emph{label} attached
to the value \texttt{123}. The label implies that the value \texttt{123}
should be used as the input parameter \texttt{x} in a particular computation.
...
to optimize the CPU load using low-level OS threads. The application
code does not need to decide how many concurrent actors to instantiate
at any given time. 
